...
function seguir() {
    async function actualizarBotonesUsuario(seguido_id, esSeguir) {
        // Selecciona todos los botones del mismo usuario
    ...
    async function manejarSeguimiento(event) {
        const button = event.currentTarget;
        const seguidor_id = button.getAttribute('data-seguidor-id');
        const seguido_id = button.getAttribute('data-seguido-id');
        const esSeguir = button.classList.contains('seguir');

        // Si va a dejar de seguir, mostrar confirmación
        if (!esSeguir) {
        ...
        try {
            const response = await enviarAjax(
                esSeguir ? 'seguir_usuario' : 'dejar_de_seguir_usuario',
                {
            ...
            if (response.success) {
                actualizarBotonesUsuario(seguido_id, esSeguir);
            } else {
                alert('Error: ' + (response.message || 'Ha ocurrido un error'));
            }
        } catch (error) {
    ...
    document.querySelectorAll('.seguir, .dejar-de-seguir').forEach(button => {
        if (!button.dataset.listenerAdded) {
            button.addEventListener('click', manejarSeguimiento);
