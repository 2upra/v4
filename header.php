        ...
        mutationsList.forEach((mutation) => {
            if (mutation.type === "attributes" && mutation.attributeName === "style") {
                ...
                if (computedStyle.display !== 'none' && computedStyle.opacity == 0) {
                    // El elemento está siendo mostrado
                    element._isTransitioning = true; // Marcar como en transición
                    element.style.display = 'block'; // Asegurarse de que esté visible
                    element.style.transition = "opacity 1s, height 1s";
                    element.style.height = element.scrollHeight + "px"; // Cambiar la altura a la altura natural
                    element.style.opacity = 1; // Iniciar la transición de opacidad
                ...
                } else if (computedStyle.display !== 'none' && computedStyle.opacity == 1) {
                    // El elemento está siendo ocultado
                    element._isTransitioning = true; // Marcar como en transición
                    element.style.transition = "opacity 1s, height 1s";
                    element.style.height = 0; // Cambiar la altura a 0
                    element.style.opacity = 0; // Iniciar la transición de opacidad

                    element.addEventListener("transitionend", function handler() {
                        element.style.display = 'none'; // Cambiar a display none después de la transición
                        element._isTransitioning = false; // Quitar la marca
                        element.style.transition = ""; // Limpiar la transición
                        element.removeEventListener("transitionend", handler);
                    });
