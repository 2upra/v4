...
{
    if (!$mostrarIcono) return esc_html($imp); // Escapar por si se muestra como texto
    $iconos = [
        'baja' => $GLOBALS['baja'] ?? 'B',
...
{
    if (!$mostrarIcono) return esc_html($tipo); // Escapar por si se muestra como texto
    $iconos = [
