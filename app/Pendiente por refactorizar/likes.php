...
function handle_post_like() {
    if (!is_user_logged_in()) { echo 'not_logged_in'; wp_die(); }
    $user_id = get_current_user_id();
    $post_id = $_POST['post_id'] ?? '';
    $nonce = $_POST['nonce'] ?? '';
    $like_state = $_POST['like_state'] ?? false;
    if (!wp_verify_nonce($nonce, 'ajax-nonce') || empty($post_id)) { echo 'error'; wp_die(); }
...
function handle_like_action($post_id, $user_id, $action) {
    ...
    if ($action === 'like' && !check_user_liked_post($post_id, $user_id)) {
        ...
        } else { error_log("Error al insertar 'me gusta': " . $wpdb->last_error); }
    } elseif ($action === 'unlike') {
        $result = $wpdb->delete($table_name, ['user_id' => $user_id, 'post_id' => $post_id]);
        if (!$result) { error_log("Error al eliminar 'me gusta': " . $wpdb->last_error); }
    }
}
