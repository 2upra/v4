    ...
    if (!empty($_POST['tags'])) {
        ...
        if (!empty($tags_array)) {
            ...
            if (is_wp_error($result)) {
                 ...
                 error_log("Error en asignarTags: Fallo al asignar tags para Post ID {$postId}. Error: " . $error_message);
            } elseif (empty($result)) {
                 // A veces retorna un array vacío si no se añadieron nuevos términos pero no es error.
                 error_log("Advertencia en asignarTags: wp_set_post_tags retornó vacío para Post ID {$postId}. Tags: " . implode(', ', $tags_array) . ". Podría ser normal si los tags no cambiaron o no existen.");
            } else {
                 error_log("Tags asignados correctamente para Post ID {$postId}: " . implode(', ', $tags_array));
            }
        } else {
             // Si después de limpiar, el array está vacío, opcionalmente eliminar todos los tags
             // wp_set_post_tags($postId, [], false);
             error_log("Info en asignarTags: No se proporcionaron tags válidos para Post ID {$postId} en el campo 'tags'.");
        }
    } else {
         // No se proporcionó el campo 'tags'
         // Opcionalmente, podrías querer eliminar todos los tags existentes si el campo está vacío:
         // wp_set_post_tags($postId, [], false);
         error_log("Info en asignarTags: Campo 'tags' no presente o vacío para Post ID {$postId}. No se asignaron tags.");
    }
}
